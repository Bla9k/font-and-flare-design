...
// Mock gacha cards (in a real app, these would come from your database)
const mockCards: Card[] = [
  // UR Cards (1% drop rate)
  { id: 1, name: "Guts Berserker Armor", image: "https://cdn.myanimelist.net/images/characters/8/359417.jpg", rarity: "UR", series: "Berserk" },
  { id: 2, name: "Miyamoto Musashi", image: "https://cdn.myanimelist.net/images/characters/12/253169.jpg", rarity: "UR", series: "Vagabond" },
  
  // SSR Cards (5% drop rate)
  { id: 3, name: "Griffith", image: "https://cdn.myanimelist.net/images/characters/5/348456.jpg", rarity: "SSR", series: "Berserk" },
  { id: 4, name: "Sasaki Kojiro", image: "https://cdn.myanimelist.net/images/characters/15/200273.jpg", rarity: "SSR", series: "Vagabond" },
  { id: 5, name: "Monkey D. Luffy", image: "https://cdn.myanimelist.net/images/characters/9/310307.jpg", rarity: "SSR", series: "One Piece" },
  { id: 6, name: "Roronoa Zoro", image: "https://cdn.myanimelist.net/images/characters/3/100516.jpg", rarity: "SSR", series: "One Piece" },
  
  // SR Cards (15% drop rate)
  { id: 7, name: "Casca", image: "https://cdn.myanimelist.net/images/characters/3/345562.jpg", rarity: "SR", series: "Berserk" },
  { id: 8, name: "Takuan Soho", image: "https://cdn.myanimelist.net/images/characters/12/76563.jpg", rarity: "SR", series: "Vagabond" },
  { id: 9, name: "Nami", image: "https://cdn.myanimelist.net/images/characters/14/61271.jpg", rarity: "SR", series: "One Piece" },
  { id: 10, name: "Vinsmoke Sanji", image: "https://cdn.myanimelist.net/images/characters/8/210265.jpg", rarity: "SR", series: "One Piece" },
  { id: 11, name: "Nico Robin", image: "https://cdn.myanimelist.net/images/characters/2/263245.jpg", rarity: "SR", series: "One Piece" },
  
  // R Cards (79% drop rate)
  ...Array(20).fill(0).map((_, i) => ({
    id: 12 + i,
    name: `Common Character ${i + 1}`,
    image: `https://cdn.myanimelist.net/img/sp/icon/apple-touch-icon-256.png`,
    rarity: "R" as Rarity,
    series: ["Berserk", "Vagabond", "One Piece"][Math.floor(Math.random() * 3)]
  }))
];
...
export default function Gacha() {
  ...
  const [collection, setCollection] = useState<{[id: number]: Card & {count: number}}>({});
  ...
  const [currentPull, setCurrentPull] = useState<Card | null>(null);
  ...
  // Pull animation sequence with card effects
  ...
  const calculatePullResult = (): Card => {
    // Apply pity system
  ...
  const performPull = async (isFreePull = false) => {
    ...
    if (!isFreePull && currency < 100) {
      toast({
        ...
        description: "You need 100 gems to pull.",
        variant: "destructive"
      });
      return;
    }
    ...
    setTimeout(() => {
      toast({
        ...
        description: `You obtained: ${result.name}`,
  ...
  const performMultiPull = async () => {
    ...
    if (currency < 900) { // 10% discount for multi-pull
      toast({
        title: "Not Enough Gems",
        description: "You need 900 gems for a 10-pull (10% discount).",
        variant: "destructive"
      });
      return;
    }
    ...
    <Layout>
      <div className="container mx-auto px-4 py-8">
        ...
        >
          ...
          <span className="text-anime-red">Gacha</span>システム
          <span className="text-anime-cyberpunk-blue">]</span>
        </motion.h1>
        ...
        >
          Test your luck with our Gacha system! Collect rare manga character cards with various rarities.
          The pity system ensures you'll get an SSR card every 50 pulls and a UR card every 100 pulls!
        </motion.p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          >
            ...
            <div className="flex flex-col items-center justify-center bg-anime-dark/60 backdrop-blur-sm border border-anime-light-gray rounded-lg p-8 mb-6 min-h-[500px] relative overflow-hidden">
              ...
              {currentPull ? (
                ...
                >
                  {/* Card container */}
                  <motion.div 
                    className={`relative w-64 h-96 rounded-xl overflow-hidden ${getRarityGlow(currentPull.rarity)}`}
                    whileHover={{ scale: 1.05 }}
                    transition={{ duration: 0.3 }}
                  >
                    {/* Card background */}
                    <div className={`absolute inset-0 ${getRarityColor(currentPull.rarity)}`}></div>
                    
                    {/* Card image */}
                    <div className="absolute inset-0 p-2 pt-10">
                    ...
                    {/* Rarity badge */}
                    <div className="absolute top-2 right-2 px-2 py-1 rounded bg-black/70 text-white text-sm font-digital">
                      {currentPull.rarity}
                    </div>
                    
                    {/* Card name */}
                    <div className="absolute bottom-0 left-0 right-0 bg-black/70 backdrop-blur-sm p-3 text-center">
                      <h3 className="text-white font-display font-bold text-lg truncate">{currentPull.name}</h3>
                      {currentPull.series && (
                        <p className="text-gray-300 text-sm">{currentPull.series}</p>
                      )}
                    </div>
                  </motion.div>
                  ...
                  <button
                    ...
                    className="mt-6 px-6 py-2 bg-anime-cyberpunk-blue text-anime-dark rounded-full font-display hover:bg-opacity-80 transition-colors"
                  >
                    View Details
                  </button>
                </motion.div>
                ...
                <div className="text-center">
                  ...
                  <p className="text-gray-400 mb-6 max-w-md">
                    Pull for rare manga character cards. The rarer the card, 
                    the more spectacular the animation!
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              >
                <div className="text-sm font-digital text-anime-red">100 GEMS</div>
                <div className="font-display font-bold">Single Pull</div>
              </button>
              ...
              >
                <div className="text-sm font-digital">900 GEMS</div>
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50">
          ...
          >
            <div className="relative">
              <div className={`h-64 ${getRarityColor(currentPull.rarity)}`}>
            ...
            <div className="p-6">
              <div className="flex justify-between items-start mb-4">
                <h2 className="text-2xl font-display font-bold">{currentPull.name}</h2>
              ...
              <div className="mb-4">
                ...
                <div className="font-digital">#{currentPull.id.toString().padStart(4, '0')}</div>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                {Object.values(collection).map((card) => (
                  ...
                  >
                    ...
                    <div className="p-2 bg-anime-dark">
                      <div className="text-sm font-display font-bold truncate">{card.name}</div>
